...
class ClassicOptions():
    ...
    def Euro(s, k, r, T, vol, q):
        ...
        vol : number of any type (int, float8, float64 etc.), numpy array of any type
            ...
            Scholes model
        q : number of any type (int, float8, float64 etc.) or numpy array of any type
        ...
        put_value = strike*norm.cdf(-d2) - option*norm.cdf(-d1)
        call_value = option*norm.cdf(d1) - strike*norm.cdf(d2)
        ...
        deriv_theta = -0.5*np.exp(-q*T)*norm.pdf(d1)*s*vol / np.sqrt(T)
        call_theta = deriv_theta - strike*norm.cdf(d2) + option*norm.cdf(d1)
        put_theta = deriv_theta + strike*norm.cdf(-d2) - option*norm.cdf(-d1)
        
        call_rho = strike*T*norm.cdf(d2)
        put_rho = -strike*T*norm.cdf(-d2)
